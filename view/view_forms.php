...
<head>
    ...
    <title>Forms</title>
    <base href="<?= $web_root ?>">
    <link rel="stylesheet" href="//fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20,400,0,0&icon_names=globe,poker_chip,public_off,search,shoppingmode">
...
<body class="d-flex flex-column align-items-center pt-5 bg-secondary-subtle" data-bs-theme="dark">
    ...
    <div class="container w-100 mt-4" style="max-width: 700px">
        <div class="card">
            ...
            <div class="d-flex  gap-3 ms-4 mb-4">
                <span class="text-body-secondary align-content-center">Filter by category:</span>
                <div id="search_color_div" class="d-flex gap-2">
                    <?php foreach (Color::cases() as $color_name): ?>
                        <div class="d-flex flex-column-reverse gap-1">
                            ...
                            <span style="color:<?= $color_name->get_hex() ?>;font-weight:bold;" class="material-symbols-outlined">
                                shoppingmode
                            </span>
                        </div>
                    <?php endforeach; ?>
    ...
    <div class="container w-100 text-secondary fs-5 fw-semibold fst-italic m-3 ps-4" style="max-width: 700px">Hi
        <span class="text-start text-secondary fs-5 fw-semibold fst-italic"><?= $user->get_full_name() ?></span>, your forms:
    </div>
                    ...
                    <div>
                        <?php foreach (Color::cases() as $color): ?>
                            <?php if (in_array($color, $form->get_color())): ?>
                                <div class="material-symbols-outlined" id="<?= $color->value . '_' . $form->get_id() ?>" style="color:<?= $color->get_hex() ?>;">shoppingmode</div>
                            <?php else: ?>
                                <div hidden id="<?= $color->value . '_' . $form->get_id() ?>" style="color:<?= $color->get_hex() ?>;">shoppingmode</div>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </div>
                </div>
                    ...
                    <div class="d-flex gap-2">
                        <?php if (!empty($form->get_questions())): ?>
                            ...
                            <form action="instance/index/<?= $encoded_search !== "" ? $encoded_search : "" ?>"
                                ...
                                <button type="submit" class="btn btn-primary" title="Answer the form">Open</button>
                            </form>
                        <?php endif; ?>
                        <?php if ($form->has_edit_access($user)): ?>
                            ...
                            <a href="form/index/<?= $form->get_id() ?>/<?= $encoded_search !== "" ? $encoded_search : "" ?>"
                                ...
                                <span class="btn btn-secondary" title="Admin access to the form">Manage</span>
                            </a>
                        <?php endif; ?>
                    </div>
                </div>
